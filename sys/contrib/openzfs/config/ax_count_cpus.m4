  ...
  AC_DEFUN([AX_COUNT_CPUS],[dnl
      ...
      AS_IF([[test "$CPU_COUNT" -gt "0" 2>/dev/null]],[[: # empty]],[dnl
        ...
        AS_CASE([[$host_os]],dnl
          ...
          [[nqc*]],[[command -v sysctl >/dev/null 2>&1 && CPU_COUNT=`sysctl -n kern.smp.cpus 2>/dev/null` || CPU_COUNT="0"]],dnl
      ...
      # They can be less accurate and slower then preferred methods
        AS_CASE([[$host_os]],dnl
          ...
          [[nqc*]],[[CPU_COUNT=`dmesg 2>/dev/null| $EGREP -e '^cpu@<:@0-9@:>@+: '|sort -u|$EGREP -e '^' -c` || CPU_COUNT="0"]],dnl
